...
object GamePublishCpUid {
  def main(args: Array[String]): Unit = {
    ...
    val hiveContext = new HiveContext(sc)
                                    //hdfs://master-yyft:9000/cpuid.txt
...
//    df.collect().foreach{println}
    ...
    FileUtil.appendToFile("/home/bigdata/cp_uid.csv",header)
    resultDf.foreachPartition(rows =>{
      for(insertedRow<-rows){

        FileUtil.appendToFile("/home/bigdata/cp_uid.csv",insertedRow.get(0)+","+insertedRow.get(1)+","+insertedRow.get(2)+"," + insertedRow.get(3) + "," + insertedRow.get(4) + "," + insertedRow.get(5))
